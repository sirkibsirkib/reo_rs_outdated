...
impl ByteSet {
    ...
    pub fn atomic_drain_byte(&self, byte_id: usize, extract: u8) -> u8 {
        ...
        let slice_u8: &[AtomicU8] = unsafe { slice_u64.align_to::<AtomicU8>().1 };
    ...
    pub fn get_byte(&self, byte_id: usize) -> u8 {
        let slice_u64 = &self.data[..];
        let slice_u8: &[UnsafeCell<u8>] = unsafe { slice_u64.align_to::<UnsafeCell<u8>>().1 };
        let cell = slice_u8.get(byte_id).expect("OUT OF BOUNDS");
        unsafe { *cell.get() }
    }
    ...
    pub fn build(self) -> Prot {
        Prot {
            ...
            msg_dropboxes: self
                .id_type
                .into_iter()
                .map(|(id, _)| (id, MsgDropbox::default()))
                .collect(),
            rules: self.rules,
